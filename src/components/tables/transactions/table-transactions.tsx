...
import TransactionBodyRow from './transactions-body-row';
import TransactionHeaderRow from './transactions-header-row';
...
export default function TableTransactions(): JSX.Element {
        ...
        <Table>
          ...
          <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
            {data?.items.map((item, index) => (
              <TransactionBodyRow
                key={item._id}
                index={index + (pageReducer.page - 1) * pageReducer.limit}
                item={item}
                onSelect={(item) => {
                ...
                onDelete={(event) => {
                  ...
                  mutate({ user: uid, id: item._id, transactionEntity: item });
                }}
              />
            ))}
          </TableBody>
